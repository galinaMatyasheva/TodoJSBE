...
module.exports.createNewTask = (req, res, next) => {
  ...
  if (!task.text) {
    return res.status(400).json({ success: false, error: "invalid data during creating task" });
  }
...
module.exports.changeTaskInfo = (req, res, next) => {
  ...
  Task.findOneAndUpdate({ _id: task._id }, task, { new: true }).then(() => {
    Task.find().then(result => {
      res.send({ data: result });
    });
  }).catch(error => {
...
module.exports.deleteTask = (req, res, next) => {
  Task.deleteOne({ _id: req.params.id }).then(() => {
    ...
    console.log("delete success one elem")
  }).catch(error => {
